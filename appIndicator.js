...
export class AppIndicator extends Signals.EventEmitter {
    ...
    async _updateAppInfo(cancellable) {
        ...
        try {
            ...
            const windowTrackerApp = Shell.WindowTracker.get_default().get_app_from_pid(pid);
            this._appInfo = windowTrackerApp?.appInfo;
            this._appId = windowTrackerApp?.get_id();
    ...
    // Shell.App id (e.g. a .desktop file id) resolved from this indicator's
    // process, once _updateAppInfo() has completed. Used to detect when a
    // legacy XEmbed tray icon belongs to the same running app as this
    // AppIndicator, so the legacy one can be suppressed in favor of this one.
    get appId() {
...
class AppIndicatorsIconActor extends St.Icon {
    ...
    _init(indicator, iconSize) {
        ...
        Util.connectSmart(settings, 'changed::icon-size-overrides', this, () =>
            this._updateWhenFullyReady());
