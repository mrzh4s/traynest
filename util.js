...
// Looks up a per-indicator override row in a flat a(s?) gsettings tuple-array
// key (uniqueId, value), returning null if this indicator has no override.
export function getIndicatorOverride(uniqueId, settingsKey) {
    const settings = SettingsManager.getDefaultGSettings();
    const overrides = settings.get_value(settingsKey).deep_unpack();
    const entry = overrides.find(([id]) => id === uniqueId);
