...
import * as IndicatorGroupManager from './indicatorGroupManager.js';
import * as IndicatorStatusIcon from './indicatorStatusIcon.js';
import * as QuickSettingsManager from './quickSettingsManager.js';
...
export class StatusNotifierWatcher {
    ...
    async _registerItem(service, busName, objPath) {
        ...
        try {
            ...
            IndicatorStatusIcon.registerIndicatorName(indicator.uniqueId,
                indicator.title || indicator.id);
            indicator.connect('accessible-name', () => {
                IndicatorStatusIcon.registerIndicatorName(indicator.uniqueId,
                    indicator.accessibleName);
            });

            const groupManager = IndicatorGroupManager.IndicatorGroupManager.getDefault();
            const orderIndex = IndicatorStatusIcon.getIndicatorOrder(indicator.uniqueId);

            if (groupManager && groupManager.shouldGroup(orderIndex)) {
                groupManager.addIndicator(indicator);
            } else {
                const statusIcon = new IndicatorStatusIcon.IndicatorStatusIcon(indicator);
                IndicatorStatusIcon.addIconToPanel(statusIcon);
            }

            // Only remove a same-app legacy tray icon once the modern one
            // above has actually been created and placed successfully --
            // never take away the fallback before the replacement exists.
            // Checked now (matchName is available immediately from D-Bus
            // properties) and again once appId resolves later, if it ever
            // does -- many tray-icon-only apps never get a Shell.WindowTracker
            // match, so matchName is often the only signal that ever fires.
            const legacyMatchDescriptor = () => ({
                appId: indicator.appId,
                matchName: (indicator.id || indicator.title)?.toLowerCase() ?? null,
            });
            TrayIconsManager.TrayIconsManager.removeLegacyCounterpart(legacyMatchDescriptor());
            indicator.connect('app-info-updated', () =>
                TrayIconsManager.TrayIconsManager.removeLegacyCounterpart(legacyMatchDescriptor()));

            const quickSettingsManager = QuickSettingsManager.QuickSettingsManager.getDefault();
            if (quickSettingsManager && !IndicatorStatusIcon.isIndicatorHidden(indicator.uniqueId))
                quickSettingsManager.mirrorIndicator(indicator);
